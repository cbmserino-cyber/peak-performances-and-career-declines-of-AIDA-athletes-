## ---------------------------------------------------------------------
## Predict when a new athlete will peak in CWT using LSTM
## ---------------------------------------------------------------------
...
df = pd.read_csv("/home/bernard/Documents/class3/aida ranking project/fimranks.csv") # main dataset 

df.replace('-', 0, inplace=True)  # replace '-' with 0

# ensure string in col convert to number, replace invalids with NaN
df["Rank"] = pd.to_numeric(df["Rank"], errors="coerce")
for col in ["Result", "Announced", "Points", "Penalties"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")

df.fillna(0, inplace=True)  # replace NaN with 0
df.dropna(how='all', inplace=True) 

# ~ df = df.sort_values(by=["Year", "Rank"]).grou3pby("Year").head(10)

df = df.sort_values(by=["Athlete", "Year"]) # sort
df = df.rename(columns={"Points": "Points_Current_Year"}) # rename col

# year when each athlete first and recent (last) competed
peak_year = df.loc[df.groupby("Athlete")["Points_Current_Year"].idxmax(), ["Athlete", "Year", "Points_Current_Year"]].rename(columns={"Year": "Best_Year", "Points_Current_Year": "Peak_Total_Points"})
...
df["Performance_Growth_Prev_Year"] = (
    df.groupby("Athlete")["Points_Current_Year"]
...
df = df.drop(columns=["Athlete","First_Year", "Best_Year", "Year","Result", "Penalties", "Date", "Event", "Country"])
...
SEQ_LENGTH = 6 # number of years in sequence
...
model = Sequential([
    Bidirectional(LSTM(128, return_sequences=True, activation="relu"), input_shape=(SEQ_LENGTH, X_train.shape[2])),
    Dropout(0.3),
...
athlete_last_3_years = pd.read_csv("/home/bernard/Documents/class3/aida ranking project/fim_last_3_years.csv") # dataset
